...
const ERCalendarSingleBase: React.FC<ERCalendarSingleProps> = ({
  ...
  const selectDateString = useCallback(
    (dateString: string) => {
      onChange(dateString);
    },
    [onChange]
  ...
  const ctxValue = useMemo<CalendarCtxValue>(
    () => ({
      onPressDateString: selectDateString,
    }),
    [selectDateString]
  );
      ...
      <View style={[styles.container, containerStyle]}>
        <View style={styles.headerRow}>
          ...
          <View style={styles.headerActions}>
            <Pressable onPress={goPrevMonth} style={styles.arrowBtn} hitSlop={8}>
              <MaterialIcon name="chevron-left" size={28} color="#111" />
            </Pressable>
            <Pressable onPress={goNextMonth} style={styles.arrowBtn} hitSlop={8}>
