...
import { useRoutineHistoryCreateMutation } from '@/hooks/common/useRoutineHistoryCreateMutation';
...
export const useRoutineProgressModal = (
  ...
  const closeRestTimerTemp = useCallback(() => {
    setRestTimerOpen(false);
  }, []);
  ...
  const addSet = useCallback((routineExerciseId: string) => {
    let createdSetId = '';
  ...
  const confirmRoutineFinish = useCallback(async () => {
    try {
